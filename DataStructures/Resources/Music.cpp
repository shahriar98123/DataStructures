...
Music :: Music(std::string data)
{
    std::stringstream parseCSV(data);
    
    std::string artist_hotttnesss, artist_id,    artist_name, artist_mbtags,    artist_mbtags_count,    bars_confidence,    bars_start,    beats_confidence,    beats_start,    duration,    end_of_fade_in,    familiarity,    key,    key_confidence,    latitude,    location,    longitude,    loudness,    mode,    mode_confidence,    release_id,    release_name,    similar,    song_hotttnesss,    song_id,    start_of_fade_out,    tatums_confidence,    tatums_start,    tempo,    terms,    terms_freq,    time_signature,    time_signature_confidence,    title,    year;
...
std::string  Music :: getArtist_id() const
{
    return artist_id;
}

std::string Music:: getArtist_name() const
{
    return artist_name;
}
std::string Music::getArtist_mbtags() const
{
...
std::string Music :: getLocation() const{   return  location; }
...
std::string Music :: getRelease_name() const{   return  release_name; }
std::string Music :: getSimilar() const{   return  similar; }
double Music :: getSong_hotttnesss() const{   return  song_hotttnesss; }
std::string Music :: getSong_id() const{   return  song_id; }
...
std::string Music :: getTerms() const{   return  terms; }
...
std::string Music :: getTitle() const{   return  title; }
int Music :: getYear() const{   return  year; }

std::ostream & operator << (std::ostream &outputStream, const Music & outputData)
{
    return outputStream << outputData.getSong_id() << " is the song " << outputData.getArtist_id() << " is the artist from " << outputData.getYear() << std::endl;;
}
